...
void InitializeProgram() {
  ...
  shaderList.push_back(
      Framework::LoadShader(GL_VERTEX_SHADER, "FragPosition.vert"));
  shaderList.push_back(
      Framework::LoadShader(GL_FRAGMENT_SHADER, "FragPosition.frag"));

  theProgram = Framework::CreateProgram(shaderList);

  windowHeightUniform = glGetUniformLocation(theProgram, "windowHeight");
}
...
void reshape(int w, int h) {
  ...
  glUseProgram(theProgram);
  glUniform1f(windowHeightUniform, h);
  glUseProgram(0);
}

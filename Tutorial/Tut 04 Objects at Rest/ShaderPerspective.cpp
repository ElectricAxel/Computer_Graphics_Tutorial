...
//GLuint planeUniform;
//float plane[] = { 0.0f, 0.0f, -1.0f };

void InitializeProgram() {
  std::vector<GLuint> shaderList;

  /*shaderList.push_back(
      Framework::LoadShader(GL_VERTEX_SHADER, "ManualPerspective.vert"));*/
  shaderList.push_back(
			Framework::LoadShader(GL_VERTEX_SHADER, "MatrixPerspective.vert"));
  shaderList.push_back(
      Framework::LoadShader(GL_FRAGMENT_SHADER, "StandardColors.frag"));
  ...
  // zFarUnif = glGetUniformLocation(theProgram, "zFar");
	// planeUniform = glGetUniformLocation(theProgram, "plane");
...
void display() {
  ...
  glUniform2f(offsetUniform, 0.5f, 0.5f);
	//glUniform3fv(planeUniform, 1, plane);
	glUniformMatrix4fv(perspectiveMatrixUnif, 1, GL_FALSE, perspectiveMatrix);
...
void keyboard(unsigned char key, int x, int y) {
  ...
  case 27:
    ...
    return;
	case 'a':
		perspectiveMatrix[12] -= 0.25f;
		break;
	case 'd':
		perspectiveMatrix[12] += 0.25f;
		break;
	case 's':
		perspectiveMatrix[13] -= 0.25f;
		break;
	case 'w':
		perspectiveMatrix[13] += 0.25f;
		break;
	case 'q':
		perspectiveMatrix[11] -= 0.25f;
		break;
	case 'e':
		perspectiveMatrix[11] += 0.25f;
		break;

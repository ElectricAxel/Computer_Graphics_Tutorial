...
ProgramData LoadProgram(const std::string &strVertexShader, const std::string &strFragmentShader)
{
	...
	data.worldToClipMatrixUnif = glGetUniformLocation(data.theProgram, "worldToClipMatrix");
...
void display()
{
	...
	if(g_pConeMesh && g_pCylinderMesh && g_pCubeTintMesh && g_pCubeColorMesh && g_pPlaneMesh)
	{
		...
		glm::mat4 temp(1.0f);
		temp = temp * persMatrix.Top();
		temp = temp * camMatrix.Top();

		glUseProgram(UniformColor.theProgram);
		glUniformMatrix4fv(UniformColor.worldToClipMatrixUnif, 1, GL_FALSE, glm::value_ptr(temp));
		glUseProgram(ObjectColor.theProgram);
		glUniformMatrix4fv(ObjectColor.worldToClipMatrixUnif, 1, GL_FALSE, glm::value_ptr(temp));
		glUseProgram(UniformColorTint.theProgram);
		glUniformMatrix4fv(UniformColorTint.worldToClipMatrixUnif, 1, GL_FALSE, glm::value_ptr(temp));
		glUseProgram(0);
		...
		if(g_bDrawLookatPoint)
		{
			...
			glUseProgram(ObjectColor.theProgram);
			glUniformMatrix4fv(ObjectColor.modelToWorldMatrixUnif, 1, GL_FALSE, glm::value_ptr(modelMatrix.Top()));
			glUniformMatrix4fv(ObjectColor.worldToClipMatrixUnif, 1, GL_FALSE, glm::value_ptr(persMatrix.Top()));
			g_pCubeColorMesh->Render();
			glUseProgram(0);
			glEnable(GL_DEPTH_TEST);
		}

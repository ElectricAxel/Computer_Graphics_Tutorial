...
void InitializeProgram() {
  ...
  shaderList.push_back(
      Framework::LoadShader(GL_FRAGMENT_SHADER, "ColorMultUniform.frag"));
...
public:
  ...
  bool IsAnimating() const { return !m_anim.empty(); }

  void UpdateTime() {
		size_t animation_size = m_anim.size();
		for (size_t _ = 0; _ < animation_size; ++_) {
			auto anim = m_anim.front();
			m_anim.pop_front();

			if (!anim.UpdateTime()) {
				m_anim.push_back(anim);
			} else {
				m_ixCurrOrient = anim.GetFinalIx();
			}
    }
  }
  ...
  void AnimateToOrient(int ixDestination) {
    ...
    Animation anim;
    anim.StartAnimation(ixDestination, 1.0f);

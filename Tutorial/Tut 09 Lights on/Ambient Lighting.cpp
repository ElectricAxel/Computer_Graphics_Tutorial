...
void InitializeProgram() {
  g_WhiteDiffuseColor =
      LoadProgram("DirVertexLighting_PN.vert", "ColorPassthrough.frag");
  g_VertexDiffuseColor =
      LoadProgram("DirVertexLighting_PCN.vert", "ColorPassthrough.frag");
  g_WhiteAmbDiffuseColor =
      LoadProgram("DirAmbVertexLighting_PN.vert", "ColorPassthrough.frag");
  g_VertexAmbDiffuseColor =
      LoadProgram("DirAmbVertexLighting_PCN.vert", "ColorPassthrough.frag");
}
...
void display() {
  ...
  if (g_pPlaneMesh && g_pCylinderMesh) {
    ...
    ProgramData &whiteDiffuse =
        g_bShowAmbient ? g_WhiteAmbDiffuseColor : g_WhiteDiffuseColor;
    ProgramData &vertexDiffuse =
        g_bShowAmbient ? g_VertexAmbDiffuseColor : g_VertexDiffuseColor;
    ...
    if (g_bShowAmbient) {
      ...
      glUniform4f(vertexDiffuse.ambientIntensityUnif, 1.0f, 1.0f, 1.0f, 1.0f);
    } else {
      glUseProgram(whiteDiffuse.theProgram);
    ...
    glUseProgram(whiteDiffuse.theProgram);
    glUniform3fv(whiteDiffuse.dirToLightUnif, 1,
                 glm::value_ptr(lightDirCameraSpace));
    glUseProgram(vertexDiffuse.theProgram);
    glUniform3fv(vertexDiffuse.dirToLightUnif, 1,
                 glm::value_ptr(lightDirCameraSpace));
    glUseProgram(0);

    {
      ...
      // Render the ground plane.
      {
        ...
        glUseProgram(whiteDiffuse.theProgram);
        glUniformMatrix4fv(whiteDiffuse.modelToCameraMatrixUnif, 1, GL_FALSE,
                           glm::value_ptr(modelMatrix.Top()));
        glm::mat3 normMatrix(modelMatrix.Top());
        glUniformMatrix3fv(whiteDiffuse.normalModelToCameraMatrixUnif, 1,
      ...
      // Render the Cylinder
      {
        ...
        if (g_bDrawColoredCyl) {
          glUseProgram(vertexDiffuse.theProgram);
          glUniformMatrix4fv(vertexDiffuse.modelToCameraMatrixUnif, 1, GL_FALSE,
                             glm::value_ptr(modelMatrix.Top()));
          glm::mat3 normMatrix(modelMatrix.Top());
          glUniformMatrix3fv(vertexDiffuse.normalModelToCameraMatrixUnif, 1,
                             GL_FALSE, glm::value_ptr(normMatrix));

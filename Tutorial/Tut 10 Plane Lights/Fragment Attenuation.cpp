...
UnlitProgData LoadUnlitProgram(const std::string &strVertexShader,
                               const std::string &strFragmentShader) {
  std::vector<GLuint> shaderList;

  shaderList.push_back(
      Framework::LoadShader(GL_VERTEX_SHADER, strVertexShader));
  shaderList.push_back(
      Framework::LoadShader(GL_FRAGMENT_SHADER, strFragmentShader));

  UnlitProgData data;
  data.theProgram = Framework::CreateProgram(shaderList);
  data.modelToCameraMatrixUnif =
      glGetUniformLocation(data.theProgram, "modelToCameraMatrix");
  data.objectColorUnif = glGetUniformLocation(data.theProgram, "objectColor");

  GLuint projectionBlock =
      glGetUniformBlockIndex(data.theProgram, "Projection");
  glUniformBlockBinding(data.theProgram, projectionBlock,
                        g_projectionBlockIndex);

  return data;
}

ProgramData LoadLitProgram(const std::string &strVertexShader,
                           const std::string &strFragmentShader) {
  std::vector<GLuint> shaderList;
  ...
  data.modelToCameraMatrixUnif =
      glGetUniformLocation(data.theProgram, "modelToCameraMatrix");
  data.lightIntensityUnif =
      glGetUniformLocation(data.theProgram, "lightIntensity");
  data.ambientIntensityUnif =
      glGetUniformLocation(data.theProgram, "ambientIntensity");

  data.normalModelToCameraMatrixUnif =
      glGetUniformLocation(data.theProgram, "normalModelToCameraMatrix");
  data.cameraSpaceLightPosUnif =
      glGetUniformLocation(data.theProgram, "cameraSpaceLightPos");
  data.windowSizeUnif = glGetUniformLocation(data.theProgram, "windowSize");
  data.maxDistanceUnif = glGetUniformLocation(data.theProgram, "maxDistance");

  GLuint projectionBlock =
      glGetUniformBlockIndex(data.theProgram, "Projection");
  glUniformBlockBinding(data.theProgram, projectionBlock,
                        g_projectionBlockIndex);
  GLuint unprojectionBlock =
      glGetUniformBlockIndex(data.theProgram, "UnProjection");
  glUniformBlockBinding(data.theProgram, unprojectionBlock,
                        g_unprojectionBlockIndex);
...
void InitializePrograms() {
  g_FragWhiteDiffuseColor =
      LoadLitProgram("FragLightAtten_PN.vert", "FragLightAtten.frag");
  g_FragVertexDiffuseColor =
      LoadLitProgram("FragLightAtten_PCN.vert", "FragLightAtten.frag");
...
glutil::ViewData g_initialViewData = {
    ...
    glm::fquat(0.92387953f, 0.3826834f, 0.0f, 0.0f), 5.0f, 0.0f};

glutil::ViewScale g_viewScale = {3.0f,          20.0f, 1.5f, 0.5f,
...
glutil::ViewPole g_viewPole =
    glutil::ViewPole(g_initialViewData, g_viewScale, glutil::MB_LEFT_BTN);
glutil::ObjectPole g_objtPole = glutil::ObjectPole(
    g_initialObjectData, 90.0f / 250.0f, glutil::MB_RIGHT_BTN, &g_viewPole);
...
void init() {
  ...
  glBindBufferRange(GL_UNIFORM_BUFFER, g_projectionBlockIndex,
                    g_projectionUniformBuffer, 0, sizeof(ProjectionBlock));

  // Bind the static buffers.
  glBindBufferRange(GL_UNIFORM_BUFFER, g_unprojectionBlockIndex,
                    g_unprojectionUniformBuffer, 0, sizeof(UnProjectionBlock));
...
static float g_fMaxDistance = 10.0f;
...
void display() {
  ...
  if (g_pPlaneMesh && g_pCylinderMesh && g_pCubeMesh) {
    ...
    glUniform4f(g_FragWhiteDiffuseColor.lightIntensityUnif, 0.8f, 0.8f, 0.8f,
                1.0f);
    glUniform4f(g_FragWhiteDiffuseColor.ambientIntensityUnif, 0.2f, 0.2f, 0.2f,
                1.0f);
    glUniform3fv(g_FragWhiteDiffuseColor.cameraSpaceLightPosUnif, 1,
                 glm::value_ptr(lightPosCameraSpace));
    glUniform1f(g_FragWhiteDiffuseColor.maxDistanceUnif, g_fMaxDistance);

    glUseProgram(g_FragVertexDiffuseColor.theProgram);
    glUniform4f(g_FragVertexDiffuseColor.lightIntensityUnif, 0.8f, 0.8f, 0.8f,
                1.0f);
    glUniform4f(g_FragVertexDiffuseColor.ambientIntensityUnif, 0.2f, 0.2f, 0.2f,
                1.0f);
    glUniform3fv(g_FragVertexDiffuseColor.cameraSpaceLightPosUnif, 1,
                 glm::value_ptr(lightPosCameraSpace));
    glUniform1f(g_FragWhiteDiffuseColor.maxDistanceUnif, g_fMaxDistance);
    glUseProgram(0);

    {
      ...
      // Render the ground plane.
      {
        ...
        glUseProgram(g_FragWhiteDiffuseColor.theProgram);
        glUniformMatrix4fv(g_FragWhiteDiffuseColor.modelToCameraMatrixUnif, 1,
                           GL_FALSE, glm::value_ptr(modelMatrix.Top()));

        glUniformMatrix3fv(
            g_FragWhiteDiffuseColor.normalModelToCameraMatrixUnif, 1, GL_FALSE,
      ...
      // Render the Cylinder
      {
        ...
        if (g_bDrawColoredCyl) {
          glUseProgram(g_FragVertexDiffuseColor.theProgram);
          glUniformMatrix4fv(g_FragVertexDiffuseColor.modelToCameraMatrixUnif,
                             1, GL_FALSE, glm::value_ptr(modelMatrix.Top()));

          glUniformMatrix3fv(
              g_FragVertexDiffuseColor.normalModelToCameraMatrixUnif, 1,
          ...
          glUniformMatrix3fv(
              g_FragWhiteDiffuseColor.normalModelToCameraMatrixUnif, 1,
              GL_FALSE, glm::value_ptr(normMatrix));
...
void keyboard(unsigned char key, int x, int y) {
  bool bChangedMaxDistance = false;
  ...
  case 'j':
    ...
    break;
  case 'I':
    g_fLightHeight += 0.05f;
    break;
  case 'K':
    g_fLightHeight -= 0.05f;
    break;
  case 'L':
    g_fLightRadius += 0.05f;
    break;
  case 'J':
    g_fLightRadius -= 0.05f;
    break;
  ...
  if (g_fMaxDistance < 1.0f)
    g_fMaxDistance = 1.0f;

  if (bChangedMaxDistance)

...
ProgramData LoadLitProgram(const std::string &strVertexShader,
  ...
  data.modelToCameraMatrixUnif =
      glGetUniformLocation(data.theProgram, "modelToCameraMatrix");
  data.modelSpaceLightPosUnif =
      glGetUniformLocation(data.theProgram, "modelSpaceLightPos");
...
void InitializePrograms() {
  g_WhiteDiffuseColor =
      LoadLitProgram("ModelPosVertexLighting_PN.vert", "ColorPassthrough.frag");
  g_VertexDiffuseColor = LoadLitProgram("ModelPosVertexLighting_PCN.vert",
                                        "ColorPassthrough.frag");
  g_FragWhiteDiffuseColor =
      LoadLitProgram("FragmentLighting_PN.vert", "FragmentLighting.frag");
  g_FragVertexDiffuseColor =
      LoadLitProgram("FragmentLighting_PCN.vert", "FragmentLighting.frag");
...
void display() {
  ...
  if (g_pPlaneMesh && g_pCylinderMesh && g_pCubeMesh) {
    ...
    glUniform4f(pVertColorProgram->ambientIntensityUnif, 0.2f, 0.2f, 0.2f,
                1.0f);
    glUseProgram(0);

    {
      ...
      // Render the ground plane.
      {
        ...
        // glm::mat4 invTransform = modelMatrix.InverseTop();
        glm::mat4 invTransform = glm::inverse(modelMatrix.Top());
        glm::vec4 lightPosModelSpace = invTransform * lightPosCameraSpace;
        glUniform3fv(pWhiteProgram->modelSpaceLightPosUnif, 1,
                     glm::value_ptr(lightPosModelSpace));
      ...
      // Render the Cylinder
      {
        ...
        if (g_bDrawColoredCyl) {
          glUseProgram(pVertColorProgram->theProgram);
          glUniformMatrix4fv(pVertColorProgram->modelToCameraMatrixUnif, 1,
                             GL_FALSE, glm::value_ptr(modelMatrix.Top()));

          glUniform3fv(pVertColorProgram->modelSpaceLightPosUnif, 1,
                       glm::value_ptr(lightPosModelSpace));

          g_pCylinderMesh->Render("lit-color");
        } else {
          glUseProgram(pWhiteProgram->theProgram);
          glUniformMatrix4fv(pWhiteProgram->modelToCameraMatrixUnif, 1,
                             GL_FALSE, glm::value_ptr(modelMatrix.Top()));
...
// pixels. This is an opportunity to call glViewport or glScissor to keep up with
// the change in size.
